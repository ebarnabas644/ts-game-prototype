...
export class SpritesheetBuilder{
    ...
    setMetaData(imagePath: string, size: { w: number, h: number} = {w: -1, h: -1}, format: string = 'RGBA8888', scale: string = "1"){
    ...
    addRow(animationName: string, startPos: { x: number, y: number }, numberOfFrames: number, horizontalInvert: boolean = false, frameSize: { w: number, h: number} = { w: -1, h: -1}){
    ...
    async build(){
        ...
        for (const key in this.spriteData.animations) {
            const isHorizontallyInverted = this.spriteData.frames[key+"0"].horizontalInvert
            resultSprites[key] = { textures: spritesheet.animations[key], options: { horizontalInvert: isHorizontallyInverted } }
        }
